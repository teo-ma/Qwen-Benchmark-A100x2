...
try:  # TensorRT-LLM runtime is optional
    import tensorrt_llm  # type: ignore
except ImportError:  # pragma: no cover - TRT-LLM only needed when engine=trtllm
    tensorrt_llm = None  # type: ignore[assignment]
    ModelRunner = None
else:  # pragma: no cover - runtime import still optional at execution time
    try:
...
def detect_trt_distributed_context() -> Tuple[int, int]:
    """Best-effort MPI rank/world-size detection for TensorRT-LLM launches."""

    def _read_env(keys: Tuple[str, ...], default: int) -> int:
        for key in keys:
    ...
    rank = _read_env(("RANK", "OMPI_COMM_WORLD_RANK", "SLURM_PROCID"), 0)
    world_size = _read_env(("WORLD_SIZE", "OMPI_COMM_WORLD_SIZE", "SLURM_NTASKS"), 1)
    if tensorrt_llm is not None:
        try:
            rank = int(tensorrt_llm.mpi_rank())
            world_size = int(tensorrt_llm.mpi_world_size())
        except Exception:  # pragma: no cover - runtime may omit MPI helpers
            pass
...
def run_benchmark(
    ...
    if engine == "hf":
        ...
        if ignore_eos:
            stop_kwargs = {"stop_words": [], "stop_token_ids": []}
        ...
        if precision.lower() != "fp16":
            raise ValueError("TensorRT-LLM path currently supports FP16 only in this script")
        if ModelRunner is None:
            raise RuntimeError("tensorrt-llm package is required when --engine trtllm")
        if not trt_engine_dir:
            raise ValueError("--trt-engine-dir is required when --engine trtllm")

        if torch.cuda.is_available():
            torch.cuda.empty_cache()
            torch.cuda.reset_peak_memory_stats()

        runtime_rank, runtime_world_size = detect_trt_distributed_context()
        distributed_rank = runtime_rank
        distributed_world_size = runtime_world_size
        emit_result = runtime_rank == 0

        runner = ModelRunner.from_dir(engine_dir=trt_engine_dir, rank=runtime_rank, debug_mode=False)

        pad_id = int(tokenizer.pad_token_id or tokenizer.eos_token_id or 0)
        # TensorRT-LLM expects each request tensor to be 1-D without an explicit batch dim.
        batch_inputs = [row.to(torch.int32).contiguous() for row in input_ids]

        generation_kwargs = {
            "batch_input_ids": batch_inputs,
            "max_new_tokens": gen_tokens,
            "temperature": 0.7,
            "top_p": 0.9,
            "end_id": -1 if ignore_eos else int(tokenizer.eos_token_id or pad_id),
            "pad_id": pad_id,
        ...
        sync_cuda()
        start = time.perf_counter()
        outputs = runner.generate(**generation_kwargs)
        sync_cuda()
        elapsed = time.perf_counter() - start

        output_ids = outputs["output_ids"].int().cpu()
        sequence_lengths = outputs["sequence_lengths"].cpu()
        total_tokens = int(sequence_lengths[0][0])
        new_tokens = max(total_tokens - prompt_tokens, 0)
        excerpt_slice = output_ids[0, 0, total_tokens - min(new_tokens, 64):total_tokens]
        excerpt_text = tokenizer.decode(excerpt_slice.tolist(), skip_special_tokens=True).strip()
        device_map_serializable = {
            "engine": "tensorrt-llm",
    ...
    if distributed_world_size is not None:
        result["distributed_world_size"] = distributed_world_size
    if not emit_result:
        return None
    return result
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Tensor parallel degree for vLLM/LMDeploy (TensorRT-LLM derives world size from MPI)",
    )
    parser.add_argument(
        "--attn-impl",
        choices=["auto", "eager", "flash_attention_2"],
        default="auto",
        help="Attention implementation hint for HF engine",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.95,
        help="Fraction of VRAM used for KV cache (vLLM) or LMDeploy cache budget",
    )
    parser.add_argument("--lmdeploy-model-path", help="Path to LMDeploy TurboMind workspace when --engine lmdeploy")
    parser.add_argument("--trt-engine-dir", help="Path to TensorRT-LLM engine directory when --engine trtllm")
    parser.add_argument("--output-json", help="Optional path to write JSON result")
    parser.add_argument(
        "--ignore-eos",
        action="store_true",
        help="Disable EOS-based early stopping (LMDeploy and TensorRT-LLM)",
    )
